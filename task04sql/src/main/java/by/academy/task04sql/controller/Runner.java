	...
	public static void main(final String[] args) {
		try {
			final Collection<Person> people = ServiceFactory.getInstance()
					.getPersonCollectionCreator()
					.create(PEOPLE_QUANTITY, MIN_AGE, MAX_AGE);
			final PersonCollectionUtilService utilService
					= ServiceFactory.getInstance()
					.getPersonCollectionUtilService();
			final Collection<Person> peopleYoungerThan21
					= utilService.selectPeopleUnderAge(people,
					UNDER_AGE_VALUE);
			final CollectionPrinter printer = ServiceFactory.getInstance()
					.getCollectionPrinter();
			...
			final Collection<Person> sortedPeople = utilService.sortPeople(
					peopleYoungerThan21,
