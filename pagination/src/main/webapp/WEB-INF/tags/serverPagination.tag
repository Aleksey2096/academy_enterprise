...
<div class="container middle pagination">
        ...
        <c:choose>
            ...
            <c:otherwise>
                <li class="leftChevron pagination"><a class="pagination"
                                                      href="${pageContext.request.contextPath}/dispatcher?command=${command}&recordsPerPage=${recordsPerPage}&currentPage=${currentPage-1}"></a>
                </li>
            </c:otherwise>
        </c:choose>
        <c:choose>
                    ...
                    <c:choose>
                        <c:when test="${currentPage ne i}">
                            <li class="pagination"><a class="pagination"
                                                      href="${pageContext.request.contextPath}/dispatcher?command=${command}&recordsPerPage=${recordsPerPage}&currentPage=${i}">${i}</a>
                            </li>
                        </c:when>
                        <c:otherwise>
            ...
            <c:when test="${currentPage le 4}">
                ...
                <li class="pagination"><a class="pagination"
                                          href="${pageContext.request.contextPath}/dispatcher?command=${command}&recordsPerPage=${recordsPerPage}&currentPage=${pagesNum}">${pagesNum}</a>
                </li>
            </c:when>
            <c:when test="${pagesNum - currentPage lt 4}">
                <li class="pagination"><a class="pagination"
                                          href="${pageContext.request.contextPath}/dispatcher?command=${command}&recordsPerPage=${recordsPerPage}&currentPage=1">1</a>
                </li>
                <li class="dots pagination"><a class="pagination" href="#"></a></li>
    ...
    <label for="selectRecordsPerPage" class="selectRecordsPerPage">
        ...
        <select id="selectRecordsPerPage" onchange="document.location=this.options[this.selectedIndex].value">
            <c:set var="recordsPerPageList" value="${[5,10,20]}"/>
            <c:forEach var="recordsPerPageItem" items="${recordsPerPageList}">
                <c:choose>
                    ...
                    <c:otherwise>
                        <option value="${pageContext.request.contextPath}/dispatcher?command=${command}&recordsPerPage=${recordsPerPageItem}&currentPage=1">
                                ${recordsPerPageItem}
                        </option>
                    </c:otherwise>
                </c:choose>
