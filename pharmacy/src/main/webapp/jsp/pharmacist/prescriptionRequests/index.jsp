...
<head>
    <meta charset="UTF-8">
    <title><fmt:message key="page.pharmacistProcessPrescriptionRequestsPage" bundle="${rb}"/></title>
...
<div class="main">
    ...
    <div class="mainTable">
        <div class="container narrow">
            <ul class="responsive-table processPrescriptionRequests">
                ...
                <c:forEach var="elem" items="${requestScope.prescriptionRequests.records}" varStatus="status">
                    <li class="table-row">
                        ...
                        <div class="col col-6">
                            <form method="post" action="${pageContext.request.contextPath}/dispatcher">
                                <input type="hidden" name="command" value="get_prescription_create_form">
                                <input type="hidden" name="previousRequestLink"
                                       value="/dispatcher?${pageContext.request.queryString}"/>
                                <c:choose>
                                    <c:when test="${requestScope.prescriptionRequests.records.size() > 1}">
                                        <input type="hidden" name="previousRequestLinkApproved"
                                               value="/dispatcher?${pageContext.request.queryString}"/>
                                    </c:when>
                                    <c:otherwise>
                                        <input type="hidden" name="previousRequestLinkApproved"
                                               value="/dispatcher?command=${requestScope.command}&orderField=${requestScope.orderObject.orderField}&orderType=${requestScope.orderObject.orderType}&searchValue=${requestScope.searchValue}&recordsPerPage=${requestScope.prescriptionRequests.recordsPerPage}&currentPage=${requestScope.prescriptionRequests.currentPage-1}"/>
                                    </c:otherwise>
                                </c:choose>
                                <input type="hidden" name="prescription_request_id" value="${elem.id}">
                                <button type="submit"><fmt:message key="table.approve" bundle="${rb}"/></button>
                            </form>
                        </div>
                        <div class="col col-7">
                            <form method="post" action="${pageContext.request.contextPath}/dispatcher">
                                ...
                                <input type="hidden" name="command" value="dismiss_prescription_request">
                                <input type="hidden" name="prescription_request_id" value=${elem.id}>
                                <button type="submit"><fmt:message key="table.dismiss" bundle="${rb}"/></button>
                            </form>
                        </div>
                    </li>
                </c:forEach>
